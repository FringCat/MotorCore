...
typedef struct
{
    volatile uint32_t this_time;    // 当前时刻的时间戳（如定时器当前计数值/系统时间）
    volatile uint32_t past_time;    // 上一时刻的时间戳（用于计算时间差）
    volatile double dt;            // 时间差（this_time - past_time，单位：s/ms，需根据计时基准统一）
}Time_t;
...
typedef struct
{
    float i_bus;            // 母线电流（注释：可选，用于功率计算与过流保护，单位：A）
    float u_bus;
    ...
    } angle_data __attribute__((packed));  // 定义联合体变量（如AngleData）
    ...
    } current_data __attribute__((packed));  // 定义联合体变量（如CurrentData）

    /* calibrate_2dir_block */
    float calibrate_2dir_block__velocity_target;
    /* calibrate_2dir_nonblock */
    float calibrate_2dir_nonblock__velocity_target;
    float calibrate_2dir_nonblock__time_init;
    float calibrate_2dir_nonblock__time_prep;
    float calibrate_2dir_nonblock__time_process;
    uint8_t calibrate_2dir_nonblock__state;
    float calibrate_2dir_nonblock__total_time;
    float calibrate_2dir_nonblock__velocity_integral;
    /* Calibrate_pole_pairs_block */
    float calibrate_pole_pairs_block__angle_step;
    /* Calibrate_pole_pairs_nonblock */
    float calibrate_pole_pairs_nonblock__velocity_target;
    float calibrate_pole_pairs_nonblock__time_init;
    float calibrate_pole_pairs_nonblock__time_prep;
    float calibrate_pole_pairs_nonblock__time_process;
    uint8_t calibrate_pole_pairs_nonblock__state;
    float calibrate_pole_pairs_nonblock__total_time;
    float calibrate_pole_pairs_nonblock__velocity_integral;
    /* Calibrate_PHASE_nonblock */
    float calibrate_phase_nonblock__ts;
    float calibrate_phase_nonblock__duty;
    float calibrate_phase_nonblock__time;
    uint8_t calibrate_phase_nonblock__state;
    int calibrate_phase_nonblock__phase_a;
    int calibrate_phase_nonblock__phase_b;
    int calibrate_phase_nonblock__phase_c;
    ...
    uint32_t calibrate_i_offset_nonblock__count;
    uint32_t calibrate_i_offset_nonblock__sample_total;
    uint32_t calibrate_i_offset_nonblock__ia_offset_raw_all;
    uint32_t calibrate_i_offset_nonblock__ib_offset_raw_all;
    uint32_t calibrate_i_offset_nonblock__ic_offset_raw_all;
    /* Openloop：0 idle / 1 run / 2 done */
    float openloop__angle_el;
    float openloop__progress;
    uint8_t openloop__state;
} Motor_DataTypeDef;
...
typedef struct
{
    ...
    Motor_ConfigTypeDef motor_config;    // 电机硬件配置实例（存储当前电机的固有参数）
    Motor_DrvTypeDef motor_drv;          // 电机驱动接口实例（绑定当前电机的硬件操作函数）
    Motor_AlgTypeDef motor_alg;          // 电机算法层实例（存储当前电机的控制中间变量与PID）
    Motor_DataTypeDef motor_data;        // 电机数据处理实例（存储当前电机的原始数据与校准参数）
} Motor_HandleTypeDef;
...
void calculate_park_n_theta(float uq , float ud , float angle_el, float *u_alpha, float *u_beta); // Park逆变换（dq→αβ电压）
...
void calculate_clark_n(float u_alpha ,float u_beta,float upower, float *ua, float *ub, float *uc); // Clark逆变换（αβ→三相电压）
void update_clark_n(Motor_HandleTypeDef *motor);                                    // 更新Clark逆变换结果到算法层
void calculate_clark(float ia ,float ib ,float ic, float *i_alpha, float *i_beta);    // Clark变换（三相电流→αβ电流）
void update_clark(Motor_HandleTypeDef *motor);                                      // 更新Clark变换结果到算法层
void calculate_park_theta(float i_alpha ,float i_beta ,float angle_el, float *id, float *iq); // Park变换（αβ电流→dq电流）
void calculate_park_sincos(float i_alpha ,float i_beta ,float sin_theta,float cos_theta, float *id, float *iq); // Park变换（αβ电流→dq电流）
...
// SVPWM相关
int calculate_svpwm_sector( float u_alpha , float u_beta );                                 // 计算电压矢量所在扇区（1~6）
int update_svpwm_sector(Motor_HandleTypeDef *motor);                                      // 更新扇区到算法层
int get_svpwm_sector(Motor_HandleTypeDef *motor);                                         // 获取当前扇区
...
void update_i_offset_block(Motor_HandleTypeDef *motor,int mode_sampling);  // 更新电流偏置-阻塞式（静止时多次采样平均）
int update_i_offset_nonblock(Motor_HandleTypeDef *motor,int mode_sampling);// 更新电流偏置-非阻塞式（静止时单次采样累加，需多次调用）
...
int update_phase_nonblock(Motor_HandleTypeDef *motor,float ia_no_order, float ib_no_order, float ic_no_order); // 更新相序-非阻塞式
