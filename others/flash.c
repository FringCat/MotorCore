...
int8_t flash_write_single_address(uint32_t start_address, uint64_t *buf, uint32_t len)
{
    ...
    {
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, uw_address, *data_buf) == HAL_OK)
        {
            uw_address += 8; // 64 bits read width,
            data_buf++;
            data_len++;
            if (data_len == len)
            {
...
void update_flash(uint32_t addr , uint64_t* buffer_write , uint32_t num)
{
    flash_erase_address(0, 1);

    flash_write_single_address(addr,buffer_write,num);
