...
#define SPI_CS          MT6835_CS_Pin
#define SPI_CS_PORT     MT6835_CS_GPIO_Port
...
static void mt6835_spi_send(uint8_t *tx_buf, uint8_t len) {
    ...
    if (status != HAL_OK) {
        MT6835_DEBUG("spi send failed %d\n\r", status);
        return;
    }
}
...
static void mt6835_spi_send_recv(uint8_t *tx_buf, uint8_t *rx_buf, uint8_t len) 
{
    ...
    status = HAL_SPI_TransmitReceive(&SPI_INSTANCE, tx_buf, rx_buf, len,0xFFFF);
    // HAL_GPIO_TogglePin(TEST1_GPIO_Port, TEST1_Pin);
    if (status != HAL_OK) 
    {
        MT6835_DEBUG("spi send_recv failed %d\n\r", status);
        return;
    }
    // wait IT
    ...
    while (HAL_SPI_GetState(&SPI_INSTANCE) != HAL_SPI_STATE_READY) 
    {
        ...
        {
            MT6835_DEBUG("spi send_recv timeout\n\r");
            return;
        }
    }
}
